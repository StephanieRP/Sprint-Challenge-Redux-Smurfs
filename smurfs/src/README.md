...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      Actions are what contain action/data for reducers to perform different events. The store is what holds state and it can be passed to various components through a function mapStateToProps.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
      Application state is the global state that get passed down from a parent component to its children through props. Component state is the state that is only used in one component and does not have to be shared with others. Its best to use component state for things like forms, etc.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
      It is what helps functions to run asynchronously instead of the standard synchronous. It has it wait for a particular state or another function to run before it is able to run.
